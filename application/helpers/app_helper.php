...
function convertDateFormat($date)
	{
	 ...
	 {  
	    $instance = &get_instance();
	    if($instance->session->userdata['superadmin']==1 || $instance->session->userdata['admin']==1  || $instance->session->userdata['client']==1)
		return true;
		else
		return false;
	 }
	...
	function splitArray($array_data,$step)
    {
	     $start=0;
		 $limit=$step;
	     $arraylength=count($array_data);
		 
	     $return_array=array();
	     
		 $remainder=($arraylength%$step);
		 $parts=(($arraylength-$remainder)/$step);
		 
		 
		 if($parts>=1)
		 {
		    for($i=0;$i<$parts;$i++,$start=$start+$limit,$limit++)
			{
			   $return_array[]=array_slice($array_data,$start,$limit);
			}
		 }
